...
const appid = "wx69325233c8ef0bd9"
const secret = "8a050303875ea8bdbf3950ac505f518d"
...
router.get("/recommend", (req, res) => {
    var page = url.parse(req.url, true).query.page || 1;
    const sql = "select * from recommend limit 4 offset " + (page - 1) * 4;
    SQLConnect(sql, [page], (result) => {
        if (result.length > 0) {
            res.send({
                status: 200,
...
router.get("/category", (req, res) => {
    ...
    const sql = "select * from goods where cate=?";
